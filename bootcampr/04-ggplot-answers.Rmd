...
library(tidyverse) # for tidy data & ggplot
data(starwars)
...
Now, let's create a barchart that counts the number of eye colors.
...
(@) Modify the plot to count the number of homeworlds in the dataset.
...
We can specify more than two variables and aesthetics if we wish. 

```{r}
ggplot(starwars, aes(x = homeworld, fill = eye_color)) +
  geom_bar()
```
...
```{r}
library(plotly)
ggplotly(ggplot(starwars, aes(x = height)) +
  geom_histogram())
```

(@) Create a histogram of the GDP values from the `gapminder` dataset.
...
Line graphs are great for illustrating trends in data over time.

```{r}
# We'll use a subset of the data
climate_subset <- climate %>% filter(Source == "Berkeley")
ggplot(climate_subset, aes(x = Year, y = Anomaly10y)) +
  geom_line()
```

(@) Using the `climate` dataset, can you create a line chart with a line for each source? (Hint: remember your color aesthetics.)

```{r}
# answer
ggplot(climate, aes(x = Year, y = Anomaly10y, color = Source)) +
  geom_line()
```

(@) How would you include a confidence interval in the line chart for the climate_subset data? (Some hints: look at `ggribbon`. You'll need a bit of math for `Anomly10y` and `Unc10y`. You'll need more than one geom.)

```{r}
# answer
ggplot(climate_subset, aes(Year, Anomaly10y)) +
  geom_ribbon(aes(ymin = Anomaly10y - Unc10y, ymax = Anomaly10y + Unc10y), alpha = 0.3) +
  geom_line()
```
...
(@) Using the `storms` dataset, how would you create a bar chart to visualize the types of storms?

```{r}
# answer 
ggplot(storms, aes(type)) +
  geom_bar()
```
...
(@) Create a plot with facets for each climate data source with the `climate` dataset.

```{r}
# answer
ggplot(climate, aes(Year, Anomaly10y, color = Source)) +
  geom_line() +
  facet_wrap(~ Source)
```
