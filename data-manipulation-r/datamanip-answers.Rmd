...
```{r}
midwest_census %>%
  mutate(percentageAfAm = totalAfAmPopulation / totalPopulation)
```

Can you calculate the percentage of the population that are slaves? Can you create a `population_nonwhite` column?

<!-- answer -->
```{r}
midwest_census %>% 
  mutate(percentage_slave = 100 * slavePopulation / totalPopulation)

midwest_census %>% 
  mutate(population_nonwhite = totalAfAmPopulation + totalAsiaPopulation + totalIndianPopulation)
```
...
```{r}
midwest_census %>% 
  select(year, totalPopulation, county, state) %>% 
  group_by(year) %>% 
...
```{r}
midwest_census %>% 
  select(year, totalPopulation, state, county) %>% 
  group_by(state) %>% 
  filter(year == 1910) %>% 
  top_n(1, totalPopulation)
```
...
What is the average number of white and average number of black people for each year since 1840?
...
midwest_census %>% 
  group_by(year) %>% 
  filter(year >= 1840) %>% 
  summarize(avg_white = mean(totalWhitePopulation),
            avg_black = mean(totalAfAmPopulation))
...
midwest_census %>% 
  group_by(year) %>% 
  summarize(avg_population = mean(totalPopulation))
```
...
<!--answer-->
```{r}
tidy_states <- states_by_year_and_race %>% 
  gather(race, population, 2:4)
tidy_states
```
