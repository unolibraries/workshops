...
```{r}
midwest_census %>% 
  select(year, state, county, totalPopulation) %>% 
  filter(year == 1830) %>% 
...
1. First, we select only the columns that we are interested in, namely, the column for the year, the total black population, the county, and the state. We do this just so that the results print out in a useful way: in a real analysis we might decide not to throw away the other columns.
...
midwest_census %>% 
  group_by(year) %>% 
  summarize(total_counties = n(),
            totalAfAm = sum(totalAfAmPopulation, na.rm = TRUE))
```

What was the average number of people in each county per year?

```{r}

```

What was the average number of black people in the Midwest per year?
